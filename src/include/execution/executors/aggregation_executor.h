    ...
    for (uint32_t i = 0; i < agg_exprs_.size(); i++) {
      switch (agg_types_[i]) {
        ...
        case AggregationType::SumAggregate:
          ...
          break;
        case AggregationType::MinAggregate:
          // 如果为第一次初始化，那么 sum 的值为 BUSTUB_INT32_MAX
          if (result->aggregates_[i].IsNull()) {
...
class AggregationExecutor : public AbstractExecutor {
 ...
 private:
  ...
  /* 初始时，表示表不为空；有值且为 false，表示还没用返回一次 true；有值且为 true，表示已经返回 true 了 */
  std::optional<bool> is_empty_table_handle_{std::nullopt};
};
}  // namespace bustub
