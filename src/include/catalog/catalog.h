...
/**
 * TableInfo 维护了一张 table 的 metadata
 *
 ...
 * 1. Catalog 是 非持久性的
 * 2. 为 executors 而设计
 * 3. 它负责处理: 表的创建、表的查找、索引的创建、5索引的查找
 *
...
class Catalog {
 public:
  ...
  /**
   * 创建一个新的 table 并且返回它的 metadata
   * @param table_name  table 的名称
   * @param schema table 的 schema
   * @param create_table_heap whether to create a table heap for the new table
   * @return table info 的 (non-owning) pointer
   */
  auto CreateTable(const std::string &table_name, const Schema &schema)
      -> TableInfo * {
    // 如果 table name 已经存在，则创建失败
  ...
  /**
   * 通过 table name 查询 table info
   * @param table_name 要查询的 table name
   * @return table info 的 (non-owning) pointer, 即这个指针指向 table info, 但不负责管理 table info 的生命周期
   */
  auto GetTable(const std::string &table_name) const -> TableInfo * {
    // 若找不到 table name，获取失败
  ...
  /**
   * 通过 table oid 查询 table info
   * @param table_oid 要查询的 table oid
   * @return table info 的 (non-owning) pointer
   */
  ...
  auto CreateIndex(const std::string &index_name, const std::string &table_name, const Schema &schema,
    ...
    if (index_type == IndexType::HashTableIndex) {
      ...
      // index = std::make_unique<BPlusTreeIndex<KeyType, ValueType, KeyComparator>>(std::move(meta), bpm_); // TODO(zhong): 被修改过
    }
