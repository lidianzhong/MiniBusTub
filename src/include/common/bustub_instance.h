...
class BustubInstance {
 ...
 public:
  /**
   * 创建 BustubInstance 实例
   * @param db_file_name DiskManager 需要的数据库文件名称
   */
 ...
 private:
  ...
  void HandleIndexStatement(const IndexStatement &stmt, ResultWriter &writer);
  void HandleExplainStatement(const ExplainStatement &stmt, ResultWriter &writer);
  void HandleVariableShowStatement(const VariableShowStatement &stmt, ResultWriter &writer);
  void HandleVariableSetStatement(const VariableSetStatement &stmt, ResultWriter &writer);
