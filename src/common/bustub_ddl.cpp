...
void BustubInstance::HandleCreateStatement(const CreateStatement &stmt, ResultWriter &writer) {
  ...
  if (!stmt.primary_key_.empty()) {
    ...
    index = catalog_->CreateIndex<IntegerKeyType, IntegerValueType, IntegerComparatorType>(
        stmt.table_ + "_pk", stmt.table_, info->schema_, key_schema, col_ids, TWO_INTEGER_SIZE,
...
void BustubInstance::HandleIndexStatement(const IndexStatement &stmt, ResultWriter &writer) {
  std::vector<uint32_t> col_ids;
  ...
  auto info = catalog_->CreateIndex<IntegerKeyType, IntegerValueType, IntegerComparatorType>(
      stmt.index_name_, stmt.table_->table_, stmt.table_->schema_, key_schema, col_ids, TWO_INTEGER_SIZE,
...
void BustubInstance::HandleExplainStatement(const ExplainStatement &stmt, ResultWriter &writer) {
  std::string output;
...
void BustubInstance::HandleVariableShowStatement(const VariableShowStatement &stmt,
                                                 ResultWriter &writer) {
  auto content = GetSessionVariable(stmt.variable_);
  WriteOneCell(fmt::format("{}={}", stmt.variable_, content), writer);
}

void BustubInstance::HandleVariableSetStatement(const VariableSetStatement &stmt,
                                                ResultWriter &writer) {
